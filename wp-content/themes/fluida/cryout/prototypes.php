...
function cryout_mobile_body_class( $classes ){
	...
	if ( preg_match( "/($keys)/i", $browser ) && (true == cryout_get_option( _CRYOUT_THEME_SLUG . '_mobileonios')) ) : $classes[] = 'mobile-ios'; endif; // extra class for iOS devices
	return $classes;
...
/* Ads skip to content link at the top of text */
if ( ! function_exists( 'cryout_skiplink' ) ):
function cryout_skiplink() { 
	?>
		<a class="skip-link screen-reader-text" href="#main" title="<?php esc_attr_e( 'Skip to content', 'fluida' ); ?>"> <?php _e( 'Skip to content', 'fluida' ); ?> </a>
	<?php 
}// cryout_skiplink()
endif;
// hooked in core.php
