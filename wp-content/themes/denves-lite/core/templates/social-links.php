...
if (!function_exists('denves_lite_socials_links_function')) {

	function denves_lite_socials_links_function() {
		...
		foreach ( $socials as $name => $attrs) { 
			...
			if (denves_lite_setting('denves_lite_'.$name.'_button')): 

				$i++;
				
				$link = 	esc_url(denves_lite_setting('denves_lite_'.$name.'_button'), array( 'http', 'https', 'tel', 'skype', 'mailto' ) );
		...
		if ( denves_lite_setting('denves_lite_rss_button') == "on" ): 
		...
		if ( $i > 0 ) {
			echo '<div class="social-links-wrapper col-md-4">' .  wp_kses($html, $allowed) . '</div>';
		}
		
	}
	
	add_action( 'denves_lite_socials_links', 'denves_lite_socials_links_function' );
